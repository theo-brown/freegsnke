...
FreeGSNKE can be loosely coupled to the [TORAX](https://github.com/google-deepmind/torax) core transport code: TORAX evolves the kinetic profiles and the current diffusion, FreeGSNKE solves the free-boundary equilibrium for the resulting `p'(ψ)` and `FF'(ψ)` profiles, and the two are interleaved and iterated to convergence over each coupling interval. The IMAS `equilibrium` IDS is used as the interchange format in both directions. See the `freegsnke.torax_coupling` module and the example notebooks `example12a - loose_coupling_with_TORAX.ipynb` (static equilibria), `example12b - loose_coupling_with_TORAX_vessel_dynamics.ipynb` (vessel-timescale evolution) and `example12c - loose_coupling_with_TORAX_ITER.ipynb` (ITER with a physics-based transport model, adaptive time stepping and shape control).

Two equilibrium providers are available for the coupling:

- `StaticEquilibriumSolver`: at each coupling interval the equilibrium is the static free-boundary solution for prescribed coil currents (the quasi-static approximation: the vessel eddy currents are assumed to have decayed). Optionally a `LinearShapeController` adjusts the coil currents at each solve so that the plasma boundary (midplane radii, axis height, X-point) keeps its targets while the profiles evolve, i.e. an ideal shape controller; without it the shape and position are free to drift with the profiles, which over long evolutions with large changes of beta and internal inductance can push the plasma onto the limiter (see `example12c - loose_coupling_with_TORAX_ITER.ipynb`).
- `EvolutiveEquilibriumSolver`: the coil and passive-structure currents are integrated on the vessel timescale (sub-millisecond implicit Euler steps of the circuit equations, driven by the applied coil voltages, see `freegsnke.metal_evolution`) inside each transport coupling interval, so that vertical displacement and eddy-current dynamics are resolved while the current diffusion remains with TORAX. A simple vertical position controller (`metal_evolution.VerticalPositionController`) is provided; note that for an elongated plasma the vertical instability must be controlled or the plasma drifts to the wall within a few growth times.

Within each coupling interval TORAX takes its own (fixed or adaptive) time steps on the geometry interpolated in time between the equilibria at the interval ends; the iteration between the codes converges the toroidal current density handed to FreeGSNKE, with under-relaxation or Anderson acceleration (`anderson_memory`) of the exchanged profiles.
